#!/usr/bin/env python3
"""
Generates a CSV samplesheet file from a directory of paired-end sequencing fastq compressed files
for the associated target-enrichment pipeline
"""
import argparse
import os
...
def write_samplesheet(filepaths, samplesheet_f, ref, host, targets):
    """
    Writes samplesheet file from list of paired-end Fastq file paths
    """
...
if __name__ == "__main__":
    ...
    parser.add_argument('-s', '--samplesheet',
                        default='samplesheet.csv', type=argparse.FileType('w'))
    ...
    if not os.path.isfile(args.ref + ".fasta"):
        raise FileNotFoundError(
            f"Reference genome fasta file does not exist. ref:{args.ref + '.fasta'}")

    if not os.path.isfile(args.host):
        raise FileNotFoundError(
            f"Host genome fasta file does not exist. ref:{args.host}")

    if not os.path.isfile(args.targets + ".bed"):
        raise FileNotFoundError(
            f"Targets bed file does not exist. ref:{args.targets + '.bed'}")
    ...
    paired_files_in_directory = [
        (os.path.join(args.input_dir, fp), os.path.join(
            args.input_dir, fp.replace('R1', 'R2')))
    ...
    write_samplesheet(paired_files_in_directory,
                      args.samplesheet, args.ref, args.host, args.targets)
